...
void Shape_ctor(Shape * const me, int16_t x, int16_t y) {
    static struct ShapeVtbl const vtbl = { /* vtbl of the Shape class */ &Shape_area_,
            &Shape_draw_
    };
...
Shape const *largestShape(Shape const *shapes[], uint32_t nShapes) {
    Shape const *s = (Shape *)0; uint32_t max = 0U;
    uint32_t i;
    for (i = 0U; i < nShapes; ++i) {
        uint32_t area = Shape_area(shapes[i]); /* virtual call */ if (area > max) {
            max = area;
            s = shapes[i]; }
    }
...
void printAllShapesAreas(Shape const *shapes[], uint32_t nShapes) {
    uint32_t i;
    for (i = 0U; i < nShapes; ++i) {
        printf ("Area: %d\n",Shape_area(shapes[i])); /* virtual call */
    }
}
