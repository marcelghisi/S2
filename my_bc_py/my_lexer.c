...
char* Lexer_GenTokens(Lexer* me){
    ...
    while (*expression != 0){
        ...
        switch (*expression) {
            case ' ':
            case '\t':
                head = add_token(head,NULL,SPACE);
            break;
            case '(':
                head = add_token(head,NULL,L_ROUND_BRACKET);
            break;
            case ')':
                head = add_token(head,NULL,R_ROUND_BRACKET);
            break;
            case '%':
                head = add_token(head,NULL,MODULE);
            break;
            case '+':
                head = add_token(head,NULL,PLUS);
            break;
            case '-':
                head = add_token(head,NULL,MINUS);
            break;
            case '*':
                head = add_token(head,NULL,MULTIPLY);
            break;
            case '/':
                head = add_token(head,NULL,DIVIDE);
            break;
            case '0':
            case '1':
            case '2':
